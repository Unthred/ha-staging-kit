...
public sealed class SystemService(KitPaths paths, SidecarRunner sidecar, DockerRunner docker)
{
    public async Task<IReadOnlyList<ContainerStatus>> GetContainersAsync(CancellationToken ct)
    {
        var kitRunning = await docker.IsContainerRunningAsync(paths.KitContainerCandidates, ct);
