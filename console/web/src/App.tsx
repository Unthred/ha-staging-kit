...
import { onboardingApi, type DeployResult, type OnboardingStatus, type TestResult } from "./api";
...
function TestButton({ label, onTest }: { label: string; onTest: () => Promise<TestResult> }) {
  ...
  return (
    <div className="test-row">
      ...
      {result && (
        <p className={result.ok ? "msg ok" : "msg err"}>{result.message}</p>
      )}
    </div>
  );
}
...
mqtt://<this-host-ip>:1883`}
          ...
          {current.id === "done" && (
            <>
              <h2>All set</h2>
              <p>Staging sidecar is configured. Use the dashboard for day-two operations (coming soon).</p>
              <button
                type="button"
                className="btn primary"
                onClick={async () => {
              ...
              {status?.isComplete && <p className="msg ok">Onboarding marked complete ✓</p>}
            </>
          )}
