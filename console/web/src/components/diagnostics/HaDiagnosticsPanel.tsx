...
export function HaDiagnosticsPanel({
  ...
  return (
    <div id="diag-ha-logs" className="diag-ha-workbench">
      <section className="card diag-ha-issues-panel">
        <header className="diag-section-head">
          <h3>Integration issues</h3>
        </header>

        <div className="deploy-lovelace-gate-list-tabs diag-ha-instance-tabs" role="tablist" aria-label="HA instance">
          <button
            type="button"
            role="tab"
        ...
        {visibleIssues.length === 0 ? (
          <p className="muted diag-log-empty">
            {INSTANCE_SOURCES[instanceTab]} integrations look healthy in config entries.
          </p>
        ) : (
          <ul className="diag-ha-issue-list">
      ...
      <section className="card diag-log-panel diag-ha-log-panel">
